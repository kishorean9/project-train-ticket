...
public enum Config implements PeConfig {
  ...
  private final static String SERVICE_ID = "pe/project.source";

  Config() {
    config = SpConfig.getSpConfig(SERVICE_ID);
    config.register(ConfigKeys.HOST, "train-ticket", "Data source host");
    config.register(ConfigKeys.PORT, 8090, "Data source port");
    config.register(ConfigKeys.SERVICE_NAME, "train ticket", "Data source service name");
